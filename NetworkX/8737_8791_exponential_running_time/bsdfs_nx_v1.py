...
def bsdfs(G, prefix, targets, k):
    ...
    stack = [prefix[-1]] # list(prefix)  # node stack; do not mutate the caller's list
    plen = len(prefix)  # where the reported edge list starts
    forbidden = set(prefix)  # node set on stack, forbidden for re-visiting
    ...
    while iters:
        h = len(stack) + plen - 1 - 1  # depth of the top node v
        for e in iters[-1]:
            w = e[1]
            if barrier[w] + h < k:  # admissible
                if w == cycle_root:  # the root is forbidden: close, do not push
                    yield edges[1:] + [e]
                    # assert shortest_distances[-1] > 1 unless in multigraph cycle mode
...
if __name__ == "__main__":
    ...
    Covers the three NetworkX call patterns:
        bsdfs(G, [s],    {s},     k)   cycles through a node        (directed)
        bsdfs(G, [u, v], {u},     k)   cycles through an edge       (undirected)
        bsdfs(G, [s],    targets, k)   simple edge paths s -> targets
    ...
    def brute_edge_cycles(G, prefix, k):
        """All extensions of `prefix` closing a cycle at prefix[0], as edge lists.

        Mirrors bsdfs cycle-mode semantics: parallel edges are distinct outputs,
        the prefix is forbidden, and the whole cycle has at most k edges.
        """
        root = prefix[0]
        forbidden = set(prefix)
        budget = k - (len(prefix) - 1)          # edges available, closing edge included
        get = (lambda v: G.edges(v, keys=True)) if G.is_multigraph() else (lambda v: G.edges(v))
        out = []
    ...
    def check_parallel_close_guard():
        """Regression: a frame that closes a cycle twice via parallel edges to the root.

        This is the only situation in which the `if shortest_distances[-1] > 1`
        guard in the close branch is reachable; on simple graphs it is dead code.
        """
        M = nx.MultiDiGraph()
        M.add_edge(0, 1)
        M.add_edge(1, 0)                        # key 0
        M.add_edge(1, 0)                        # key 1, parallel -> second close at the same frame
        bsdfs(M, [0], None, 3)

        got = norm(bsdfs(M, [0], None, 3))
        want = norm(brute_edge_cycles(M, [0], 3))
        assert got == want, (got, want)
        assert len(got) == 2, got               # both parallel closings reported
        assert len({tuple(e[0] for e in E) for E in bsdfs(M, [0], None, 3)}) == 1  # same node seq

        # and via a longer prefix
        got = norm(bsdfs(M, [0, 1], None, 3))
        assert got == norm(brute_edge_cycles(M, [0, 1], 3)), got
        assert len(got) == 2, got
        print("parallel-close guard: reachable and correct (2 cycles, 1 node sequence)")
    ...
    def run(trials=600, n=6, seed=20260820):
        ...
        for trial in range(trials):
            ...
            # cycle mode, edge level, for every graph class (multigraphs included)
            t = rng.randrange(n)
            got = norm(bsdfs(G, [t], None, k))
            assert got == norm(brute_edge_cycles(G, [t], k)), (trial, "edge cycles vs brute", cls.__name__, list(G.edges(keys=True) if G.is_multigraph() else G.edges()), t, k)
            if G.is_multigraph() and any(len(G[u][t]) > 1 for u in list(G.pred[t] if G.is_directed() else G.adj[t]) if u != t):
                double_close += 1
        ...
        print(f"OK: {trials} trials, n={n}; {src_in_tgt} with source in targets, "
            f"{double_close} multigraphs with parallel edges into the cycle root.")


    if __name__ == "__main__":
        check_parallel_close_guard()
        run()
